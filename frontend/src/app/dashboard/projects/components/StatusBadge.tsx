...
const META: Record<string, { label: string; cls: string; Icon: any }> = {
  draft:     { label: "مسودة",     cls: "bg-raised text-ink-muted border border-edge",     Icon: FileEdit },
  active:    { label: "نشط",       cls: "bg-green-100 text-green-800 border border-green-200",  Icon: Play },
  on_hold:   { label: "متوقف",     cls: "bg-amber-100 text-amber-800 border border-amber-200",Icon: Pause },
  completed: { label: "مكتمل",     cls: "bg-primary-100 text-primary-800 border border-primary-200",     Icon: CheckCircle },
...
export default function StatusBadge({ status, withIcon = true }: { status?: string | null; withIcon?: boolean }) {
  const meta = (status && META[status]) || { label: status || "—", cls: "bg-raised text-ink-muted border border-edge", Icon: FileEdit };
