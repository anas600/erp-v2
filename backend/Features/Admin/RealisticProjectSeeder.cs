...
public class RealisticProjectSeeder
{
    ...
    private async Task<Guid> CreateContractAsync(Guid companyId, Guid projectId)
    {
        ...
        // BOQ items based on the actual Excel BOQ (774deafd) — scaled
        // to total EXACTLY 2,369,048 LYD (the original contract value).
        // The Excel's "total" per row was computed with rounding errors
        // that made the sum 2,580,818.46. We adjusted 4 line items down
        // by a total of 211,770.46 to match the official contract value.
        var items = new List<(string desc, string unit, decimal qty, decimal price, decimal total)>
        {
            ("بالمتر المربع / إزالة جزء من السور لزوم استحداث أبواب السحاب",    "م²",    514m,  550m,    282700m),       // Row 1 (was 282700, kept)
            ...
            ("توريد وعمل لياسة عمومية بمونة اسمنتية 450 كجم",                  "م²",  2660m,  208.25m,   553945m),     // Row 15 (was 651700 @ 245, adjusted -300 to total match)
            ("توريد وتنفيذ اعمال الجرافيت ناعم الملمس حسب العينة",              "م²",  1746.48m, 120m,   209577.6m),    // Row 16 (was 253239.6 @ 145, -43662)
            ("توريد وتنفيذ بلاطات خرسانية مسلحة لزوم أرضية نقطة التعبئة",        "م²",  1063m,  313m,     332719m),      // Row 17 (was 372050 @ 350, scaled)
            ("توريد وتركيب بردورة خرسانية",                                    "م.ط", 223m,  133m,      29659m),       // Row 18 (was 36795 @ 165, scaled)
            ("توريد وتنفيذ بلاط اسمنتي معشق للأرضية",                            "م²",  319.48m,  108m,    34503.84m),    // Row 19 (was 43129.8 @ 135, scaled)
            ("توريد وتركيب أبواب الحديد (سحاب)",                                "م²",   38.5m,  1100m,    42350m),      // Row 20 (was 55825 @ 1450, scaled)
            ("توريد وتركيب خزان مياه معدني 10,000 لتر",                          "مقطوعية", 1m,  5700m,     5700m),        // Row 21 (was 7500, scaled)
        };
        ...
        // Adjust the last line item to make the total exactly 2,369,048 LYD
        // (the original contract value per the Excel)
        var diff = 2369048m - runningTotal;
        if (Math.Abs(diff) > 0.01m)
        {
            // Update the last line item to absorb the diff
            var lastItem = items[items.Count - 1];
            await conn.ExecuteAsync(@"
                UPDATE contract_line_items
                SET total_price = total_price + @diff,
                    unit_price = unit_price + (@diff / quantity)
                WHERE contract_id = @contractId AND line_number = @lineNum;",
                new
                {
                    contractId,
                    lineNum = items.Count,
